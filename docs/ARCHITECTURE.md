...
AI provides validation, categorization, extraction, and explanations. On submission, the complete problem payload is evaluated—not only its title—and a completed advisory result may move a problem to AI_VALIDATED or AI_REJECTED. Persisted decision codes distinguish `VALIDATED`, `REJECTED_IRRELEVANT`, and `REJECTED_DUPLICATE`; duplicate decisions use the same traceable problem embeddings as matching and are based on semantic similarity plus a deterministic development overlap guard. A failed or pending analysis leaves it at SUBMITTED and can be retried. AI never approves or rejects a problem on behalf of the Ministry. Validated problems are published to the submitter community feed, and only the configured support threshold makes them eligible for Ministry review. Ministry approval/rejection remains an explicit, attributable lifecycle action.
