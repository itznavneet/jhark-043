...
export function CommunityProblemFeed({ accessToken }: { accessToken: string }) {
  ...
  async function vote(problemId: string, voteType: "UPVOTE" | "DOWNVOTE") {
    ...
    try {
      ...
      setItems((current) =>
        ...
        updated.currentStatus === "MINISTRY_REVIEW"
  ...
  return (
    <section aria-labelledby="community-posts-title">
      <div className="mb-5 rounded-2xl border border-blue-100 bg-gradient-to-r from-blue-50 via-white to-orange-50/40 p-5 sm:p-6">
        ...
        <div className="mt-1 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
              Review community challenges that have passed AI validation, add your
              support, and help eligible problems reach Ministry review.
            </p>
          </div>
          <span className="status-badge status-neutral shrink-0">
            Ministry makes the final decision
          </span>
        </div>
      </div>

      <div className="mb-5 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-end sm:justify-between">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {visibleItems.map((item) => (
          <article
            className="panel flex flex-col border-blue-100 p-5 shadow-[0_16px_34px_-26px_rgba(11,79,156,0.7)] transition hover:-translate-y-0.5 hover:border-primary sm:p-6"
            key={item.id}
          >
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-slate-500">
              ...
              <span>
                Shared by {item.submitter.organizationName ?? item.submitter.displayName}
              </span>
            </div>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2 text-sm font-semibold text-ink">
              <span
                className="rounded-full bg-blue-50 px-3 py-1.5 text-primary"
                aria-label={`${item.upvoteCount} of ${threshold} upvotes`}
              >
                {item.upvoteCount} / {threshold} community support
              </span>
              {item.isOwnProblem && item.downvoteCount !== undefined ? (
                <span className="text-xs font-normal text-slate-500">
                  · {item.downvoteCount} private downvotes
                </span>
              ) : null}
            </div>
            <div className="mt-4 flex flex-1 flex-col justify-end gap-3 sm:flex-row sm:items-center sm:justify-between">
              {item.currentStatus === "MINISTRY_REVIEW" ? (
                <span className="rounded-lg bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700">
                  {item.location ?? item.district ?? "Location not specified"} · Community threshold reached — Awaiting Ministry review
                </span>
              ) : (
                <span className="text-xs text-slate-500">
                  {item.location ?? item.district ?? "Location not specified"} · Open for community support
                </span>
              )}
              <div className="flex flex-wrap items-center gap-2">
                <button
                  aria-label={
                    item.userVote === "UPVOTE" ? "Remove upvote" : "Upvote problem"
                  }
                  ...
                  disabled={
                    item.isOwnProblem ||
                    item.currentStatus !== "AI_VALIDATED" ||
                    busyId === item.id
                  }
                  onClick={() => void vote(item.id, "UPVOTE")}
                  type="button"
                >
                  👍 Upvote{item.userVote === "UPVOTE" ? " · selected" : ""}
                </button>
                <button
                  aria-label={
                  ...
                  type="button"
                >
                  👎 Downvote{item.userVote === "DOWNVOTE" ? " · selected" : ""}
                </button>
                {item.isOwnProblem ? (
                  <span className="text-xs text-slate-500">
                    Your problem cannot be voted on
                  </span>
                ) : null}
              </div>
            </div>
      ...
      {!visibleItems.length ? (
        <EmptyState
          title={items.length ? "No posts match this filter" : "No validated posts yet"}
