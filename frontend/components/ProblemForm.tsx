    ...
    >
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Provide enough context for Ministry review and future university
          matching.
        </p>
      </div>
      {fields.map(([field, label, placeholder]) => (
        <label className="block text-sm font-medium text-slate-700" key={field}>
          {label}
          <input
            className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:border-accent"
      ...
      <label className="block text-sm font-medium text-slate-700">
        Description
        <textarea
          className="mt-1.5 min-h-32 w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:border-accent"
        ...
        <span
          ...
          id="description-help"
        >
          At least 50 characters. Include who is affected and what outcome is
          needed.
        </span>
      </label>
      ...
      <label className="block text-sm font-medium text-slate-700">
        ...
        <input
          ...
          onChange={(event) => update("villageLocality", event.target.value)}
        />
      </label>
      <div>
        <div className="grid gap-4 sm:grid-cols-2">
      ...
      <label className="block text-sm font-medium text-slate-700">
        Societal context
        <textarea
          className="mt-1.5 min-h-24 w-full rounded-lg border border-slate-300 px-3 py-2"
          value={form.societalContext}
          onChange={(event) => update("societalContext", event.target.value)}
        />
      </label>
      <label className="block text-sm font-medium text-slate-700">
        Desired outcome
      ...
      <fieldset className="space-y-3 rounded-xl border border-slate-200 p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <label className="text-sm font-medium text-slate-700">
            ...
            <input
              ...
              onChange={(event) => update("evidenceTitle", event.target.value)}
            />
          </label>
        </div>
        <label className="block text-sm font-medium text-slate-700">
        ...
        <p className="text-xs text-slate-500">
          Binary uploads will be connected to the provider-neutral storage layer
          in a later phase.
        </p>
      </fieldset>
      {error ? (
      ...
      <button
        className="rounded-lg bg-accent px-4 py-2.5 font-semibold text-white hover:bg-teal-800 disabled:opacity-60"
        disabled={submitting}
        type="submit"
      >
